...
def _call_bold_ideas(
    ...
    prompt = (
        ...
        f"- If sample posts AND distinctive entities are BOTH present, you MUST find at least 1 bold idea. "
        f"NONE is not a valid response in that case.\n"
        f"- The §21b critical-mass gate applies to LIABILITY recommendations only. A POSITIVE amplification "
        f"based on a real cited entity is always allowed even if no negative topic reaches theme tier.\n"
        f"- Bold ideas can amplify positives (talent involvement, press coverage, organic genre "
        f"comparisons, community events) just as legitimately as they can flag liabilities.\n"
        f"- Respond with the SINGLE LINE 'NONE' ONLY when sample posts are empty AND distinctive entities "
        f"are empty (i.e. you have nothing real to anchor to).\n\n"
        f"Data ({window_label}):\n"
        f"Positive topics: {pos_str}\n"
        f"Negative topics: {neg_str}\n"
        f"Neutral topics: {neu_str}\n"
