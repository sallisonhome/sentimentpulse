...
test("collector, real HTTP leaderboard, dashboard mapping, zeroes, stale failures and source isolation", async () => {
  ...
  try {
    ...
    for (const [id,saber] of [["998",0],["997",1]] as const) {
      db.prepare(`INSERT INTO demo_titles(steam_app_id,name,discovered_via,is_active,is_saber_published,deactivated_at,first_seen_at,created_at,updated_at)
        VALUES(?,?,'test',0,?,?,?,?,?)`).run(id,`Retired ${id}`,saber,stamp,stamp,stamp,stamp);
      const retiredId=db.prepare("SELECT id FROM demo_titles WHERE steam_app_id=?").get(id).id;
      db.prepare(`INSERT INTO steam_review_history(app_id,bucket_start,bucket_granularity,recommendations_up,recommendations_down,source_endpoint,created_at)
        VALUES(?,?,'day',99,1,'fixture',?)`).run(id,Math.floor(Date.now()/1000)-60,stamp);
      db.prepare(`INSERT INTO demo_window_estimates_daily(demo_title_id,window,as_of_date,units_mid,review_delta,method,created_at)
        VALUES (?,'ltd',?,13000,100,'review_delta_multiplier',?)`).run(retiredId,stamp.slice(0,10),stamp);
    }
    const toxicId=db.prepare("SELECT id FROM demo_titles WHERE steam_app_id='4354730'").get().id;
    db.prepare("UPDATE demo_download_actuals SET downloads=332853 WHERE steam_app_id='4354730' AND window='ltd'").run();
    db.prepare("INSERT INTO demo_ccu_snapshots(demo_title_id,captured_at,ccu) VALUES(?,?,9999)").run(toxicId,stamp);
    for (const feed of ["top","new"]) db.prepare(`INSERT INTO demo_discovery_ranks(feed,demo_title_id,source_rank)
      VALUES (?,?,1)`).run(feed,toxicId);
