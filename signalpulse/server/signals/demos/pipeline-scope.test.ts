  ...
  try {
    ...
    globalThis.fetch = async input => {
      ...
      if (url.includes("/appreviewhistogram/")) {
        ...
        assert.ok(["5184670","4010800","4354730","3462370","4010830","4047990"].includes(id), `ineligible histogram: ${id}`);
      ...
      if (url.includes("/GetNumberOfCurrentPlayers/")) {
        ...
        assert.ok(["5184670","4010800","4354730","3462370","4010830","4047990"].includes(id), `ineligible CCU: ${id}`);
    ...
    assert.equal(daily.eligibility.eligible, 6);
    assert.equal(daily.eligibility.excluded, 8);
    assert.equal(daily.eligibility.failed, 1);
    assert.equal(daily.reviewHistory.ingested, 6);
    assert.equal(daily.ccu.succeeded, 6);
    ...
    assert.equal(body.result.estimates.rowsWritten, 30);
    ...
    assert.deepEqual(db.prepare(`SELECT DISTINCT t.steam_app_id FROM demo_window_estimates_daily e
      ...
      .map((r: any) => r.steam_app_id), ["3462370","4010800","4010830","4047990","4354730","5184670"]);
